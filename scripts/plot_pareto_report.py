...
Verified points:
  Dense neural baselines (W&B `bx7hylp6`/`8haq7zuz`/`gpmfhn6k`):
    ...
    small : avg BLER 0.931, FLOPs   599 M ( 37.3%)
...
dense = [
    ...
    ("dense_small", 37.3, 0.931, C_DENSE),
    ("dense_large", 100.0, 0.901, C_DENSE_DARK),
]
ax.plot([p[1] for p in dense], [p[2] for p in dense], "-", color=C_DENSE, alpha=0.35, linewidth=1.5, zorder=2)
for label, x, y, c in dense:
    ax.scatter(x, y, marker="s", s=90, color=c, zorder=6, edgecolors="white", linewidths=1.0)
    if "nano" in label:
        ax.annotate(label, (x, y), xytext=(8, -2), textcoords="offset points", fontsize=9, color=c)
    elif "small" in label:
        ax.annotate(label, (x, y), xytext=(8, -2), textcoords="offset points", fontsize=9, color=c)
    else:
        ax.annotate(label, (x, y), xytext=(-8, -3), textcoords="offset points", fontsize=9, color=c, ha="right")
...
ax.scatter(
    [p[0] for p in alpha_sweep],
    [p[1] for p in alpha_sweep],
    marker="o",
    s=55,
    facecolors="white",
    edgecolors=C_TRAINED,
    linewidths=1.4,
    zorder=5,
...
ax.scatter(
    [p[0] for p in mode_b[:1] + mode_b[2:]],
...
ax.annotate(
    ...
    ha="right",
    fontweight="bold",
    arrowprops=dict(arrowstyle="->", color=C_HEADLINE, lw=1.0, connectionstyle="arc3,rad=-0.2"),
)

# LMMSE classical reference (horizontal line)
ax.axhline(
    0.900, color=C_LMMSE, linestyle=":", linewidth=1.5, zorder=2, label="LMMSE LS-MRC (classical, no neural compute)"
)

# Pareto frontier through the four dominating neural points
pareto_pts = sorted([(20.0, 0.951), (37.3, 0.931), (47.3, 0.902), (100.0, 0.901)])
ax.plot(
    [p[0] for p in pareto_pts], [p[1] for p in pareto_pts], "-", color=C_HEADLINE, alpha=0.18, linewidth=2.5, zorder=1
)
...
ax.invert_yaxis()
ax.grid(True)
leg = ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.18), ncol=1, frameon=False, fontsize=8.5)
